...
//version2
let totalSalary = customersSalary.reduce(
...
//version 1
let filtered_array = filterUser.filter((user) => user.friends.length >= 8);
...
//version 1
let allFriends = mappedUsers.map((user) => user.friends).flat();
...
//version  1
const aggregateData = mappedUsers.map((user) => {
  return {
    _id: user._id,
    name: user.name,
...
for(let user of mappedUsers){
    aggregate_data.push(user._id);
    aggregate_data.push(user.name);
    aggregate_data.push(user.isActive);
}
