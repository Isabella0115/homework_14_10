...
function totalSalary(customersSalary) {
  let sum = 0;
...
function totalSalary2(customersSalary) {
  return customersSalary.reduce(
...
function filtered_array(filterUser) {
  return filterUser.filter((user) => user.friends.length >= 8);
}

//version 2
function filteredArray(filterUser) {
  let filteredArray = [];
  for (let user of filterUser) {
    if (user.friends.length >= 8) filteredArray.push(user);
  }
...
//version 1
function allFriends(appedUsers) {
  return mappedUsers.map((user) => user.friends).flat();
}

//version 2

function all_friends(mappedUsers) {
  let allFriends = [];
  for (let user of mappedUsers) {
    all_friends.push(user.friends);
  }
  return allFriends;
}
...
//version  1
function aggregateData(mappedUsers) {
...
//version  2
function aggregateData2(mappedUsers) {
